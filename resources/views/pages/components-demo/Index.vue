  ...
  <div class="layout has-sidebar dark:bg-black-600 max-w-">
    <transition name="slide">
      <aside
        ...
        class="sticky justify-center w-64 h-auto pt-6 pb-32 overflow-auto md:flex bg-primary-100 dark:bg-black-500 dark:text-gray-100"
      >
        <div class="w-full py-4">
          ...
          <vue3-router-tree :items="styles">
            <template #item="{ item }">
              <router-link
                exact-active-class="text-primary-700 dark:text-primary-300"
                :to="item.path"
              >
                <div class="flex items-center space-x-2">
                  <IconTextFill
                    class="text-gray-500 hover:transform hover:rotate-45 dark:text-gray-100"
                    height="16"
                    width="16"
                  />
                  <span
                    class="text-sm font-light text-black-900 dark:text-gray-100 whitespace-nowrap"
                  >
                    {{ item.name }}</span
                  >
                </div>
              ...
              >
                <div class="flex items-center space-x-2">
                  <IconLayers
                    ...
                    width="16"
                  />
                  <span class="text-sm font-light text-black-900 dark:text-gray-100">
                    {{ item.name }}</span
                  >
                </div>
              </router-link>
    ...
    <section
      class="w-full overflow-x-hidden transition-all duration-500 layout"
      style="overflow-y: auto; max-height: calc(100vh - 64px)"
    >
      <div class="sticky top-0 z-20 flex items-center w-full header">
        <a
          href="#"
          class="p-4 bg-gray-100 dark:bg-black-600"
        ...
        >
          <IconQueryQueue
            ...
            class="transition-transform duration-600 text-primary-500 dark:text-primary-200"
          />
        </a>

        <w-breadcrumb
          :path="$route.path"
          class="font-light text-gray-400 dark:text-gray-100 dark:bg-black-600"
        >
          <template #separator>
...
export default defineComponent({
  ...
  computed: {
    components() {
      return routes.find((route) => route.path.includes("/components"))?.children;
    },
    collapseBtnStyle() {
      return {
  ...
  mounted() {
    this.components = this.generatePath(componentsRoutes.children, "/components");
    this.layouts = this.generatePath(layoutCompRoutes.children, "/layouts");
    this.styles = this.generatePath(styleRoutes.children, "/styles");
  },
});
</script>
