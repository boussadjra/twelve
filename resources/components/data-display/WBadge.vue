<template>
  <div class="relative top-0 badge">
    ...
    <div v-else class="relative">
      <div
        class="z-10 border-2 border-white dark:border-black-600"
        :class="[...classes, ...absoluteClass]"
        :style="styles"
      ></div>
      <span
        v-if="ping"
        :class="[...classes]"
        :style="styles"
        class="absolute w-4 h-4 rounded-full opacity-75 animate-ping bg-primary-400"
      >
      </span>
    </div>
  </div>
...
import { colorable, shaped, sizeable } from "@/mixins";
...
export default defineComponent({
  ...
  computed: {
    classes(): Array<string> {
      ...
      let colors = `bg-primary-700 dark:bg-primary-400 text-white`;
      if (this.bgColor) {
